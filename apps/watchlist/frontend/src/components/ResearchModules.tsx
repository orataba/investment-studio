...
function SavedFigure({ instrumentId, notebookVersionId, source }: { instrumentId: string; notebookVersionId?: string; source: NotebookSource }) {
  const [saved, setSaved] = useState<SavedResearchSource | null>(null)
  const [error, setError] = useState('')
  useEffect(() => {
    const controller = new AbortController()
    setSaved(null); setError('')
    void getSavedResearchSource(instrumentId, source.source_id, controller.signal, notebookVersionId).then(value => {
  ...
  }, [instrumentId, notebookVersionId, source.source_id])
  return error ? <p role="alert">数值依据暂时无法读取：{error}</p> : saved ? <EvidenceFigure source={saved} /> : <p className="sector-research-note" role="status">正在读取留存数值依据…</p>
}
...
export default function ResearchModules({ instrumentId, notebook, plan, onAskAssistant }: {
  ...
  return <section className="research-domain-modules" aria-label="分领域研究">
    ...
    <nav className="research-module-index" aria-label="研究领域目录">{keys.map(key => <a key={key} href={`#${anchor(key)}`}>{plan?.modules.find(module => module.id === key)?.title || moduleTitles[key] || key}</a>)}</nav>
    {keys.map(key => {
      ...
      return <article key={key} id={anchor(key)} className="research-domain-module">
        ...
        {result ? <>
          ...
          {result.analysis && <p className="research-module-analysis" translate="no">{result.analysis}</p>}
          ...
          {figures.map(source => <SavedFigure key={`${source.source_id}:${notebook?.version_id || ''}`} instrumentId={instrumentId} notebookVersionId={notebook?.version_id} source={source} />)}
