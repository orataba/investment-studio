...
type ResearchResponse = { available: boolean; message?: string; sectors: SectorResearch[]; events: SectorEventRecord[] }
...
function ResearchRunSources({ review, instruments, expanded }: { review: Review; instruments: string[]; expanded: boolean }) {
  ...
  useEffect(() => {
    ...
    setError('')
    void fetchJson<{ web_evidence?: Array<{ operation: string; sources?: Array<EventSource & { text?: string }> }> }>(
      ...
      for (const capture of context.web_evidence || []) {
        if (capture.operation !== 'fetch') continue
        for (const source of capture.sources || []) {
          const url = sourceHref(source.url)
          if (url && typeof source.text === 'string' && source.text.trim()) originals.set(url, {
  ...
  return <section className="sector-run-sources" aria-label="本轮查阅原文">
    <h4>本轮查阅原文</h4>
...
function EventGroup({ event, progress, supplementary = false, planned = false, onAskAssistant }: { event: SectorEventRecord; progress: Progress[]; supplementary?: boolean; planned?: boolean; onAskAssistant?: (question: string) => void }) {
...
export default function SectorResearchPanel({ instrumentId, watchlistId, variant = 'timeline', onOpenEvents, onAskAssistant }: {
  ...
  onOpenEvents?: () => void; onAskAssistant?: (question: string) => void
}) {
  ...
  const statusSummary = busy ? '研究更新中'
    : data.sectors.some((sector) => sector.latest_review?.status === 'failed') ? '最新更新未完成'
      : data.sectors.some((sector) => sector.latest_review?.status === 'limited') ? '已更新，覆盖受限'
        : data.sectors.some((sector) => !completedReview(sector)) ? '尚未完成研究' : '研究已更新'
  const end = today()
  ...
  const hasCompletedResearch = data.sectors.some((sector) => completedReview(sector))
  const completedRuns = new Map<string, { review: Review; instruments: string[] }>()
  for (const sector of data.sectors) {
    const completed = completedReview(sector)
    if (!completed) continue
    const group = completedRuns.get(completed.run_id) || { review: completed, instruments: [] }
    group.instruments.push(sector.ticker || sector.sector_name)
    completedRuns.set(completed.run_id, group)
  }
  ...
  return <section className={`sector-research-panel sector-research-${variant}`} aria-label={title}>
    <header className="sector-research-heading">
      <div className="sector-research-title"><h2>{title}</h2><span>{data.sectors.length === 1 ? data.sectors[0].sector_name || data.sectors[0].ticker : `${data.sectors.length} 个标的`} · {statusSummary}</span></div>
      <div className="sector-research-actions">
        {variant === 'summary' ? onOpenEvents && <button onClick={onOpenEvents}>查看研究追踪</button> : <>
          ...
          <button onClick={() => void start()} disabled={busy || !data.available}>{busy ? '研究更新中…' : '更新研究'}</button>
        </>}
      </div>
    </header>
    <p className="sector-research-note">每天自动更新，根据基本面、事件冲击与相关数据整理重要变化。</p>
    {!data.available && <p className="sector-research-limitation">{data.message || '研究所需来源暂不可用，已保存的研究仍可查看。'}</p>}
    ...
    {variant !== 'status' && <section className="sector-current-conclusion" aria-label="当前研究结论">
      ...
      {data.sectors.map((sector) => {
        ...
        return <article key={sector.instrument_id}>
          ...
          {completed ? <>
            <p className="sector-conclusion-date">研究截至 <time dateTime={completed.checked_at || undefined}>{time(completed.checked_at)}</time></p>
            <p className="sector-conclusion-body" translate="no">{completed.summary || '本轮研究已完成，尚未保存研究结论。'}</p>
          </> : <p className="sector-research-note">尚未完成研究。</p>}
    ...
    {variant === 'timeline' && instrumentId && <ResearchDossierPanel key={instrumentId} instrumentId={instrumentId} reviewRunId={completedReview(data.sectors[0])?.run_id} onAskAssistant={onAskAssistant} />}
    ...
    {variant !== 'status' && (hasCompletedResearch || data.events.length > 0) && <section className="sector-current-risks" aria-label="当前风险与机会">
      <h3>风险与机会</h3>
      {visibleRisks.length ? visibleRisks.map((event) => <article className="sector-current-risk" key={event.case_id}>
      ...
      </article>) : <p className="sector-research-note">当前没有保留需要重点提示的风险或机会。</p>}
    ...
    {variant === 'timeline' && (hasCompletedResearch || data.events.length > 0) && <section className="sector-research-progress" aria-label="研究进展">
