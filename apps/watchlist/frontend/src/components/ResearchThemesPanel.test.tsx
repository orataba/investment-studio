...
it('preserves PM authorship and exact historical references in an actual theme timeline', async () => {
  ...
  expect(within(pm).getByText(opinion.body).closest('details')).toBeNull()
...
it('reads analysis and evolution continuously and restores linked or archived themes', async () => {
  ...
  expect(screen.getByRole('region', { name: '主题分析与演变' })).toBeTruthy()
