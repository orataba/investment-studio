...
it('shows synthesized priorities without surfacing old independent research questions', async () => {
  ...
  expect(screen.getByText(theme().priority_reason!)).toBeTruthy()
...
it('preserves PM authorship and exact historical references in an actual theme timeline', async () => {
  ...
  await screen.findByRole('heading', { name: theme().title })
  expect(container.querySelector('.research-timeline')).toBeNull()
  fireEvent.click(screen.getByRole('tab', { name: '演变' }))
  ...
  expect(within(pm).queryByRole('button', { name: '追问这条更新' })).toBeNull()
  fireEvent.click(within(pm).getByText('阅读这条记录'))
...
it('reads one theme at a time, supports keyboard sections and restores linked or archived themes', async () => {
  ...
  window.history.replaceState(null, '', '/instruments/gold?tab=research#research-theme-second')
  const { container } = render(<ResearchThemesPanel instrumentId="gold" />)
  await screen.findByRole('heading', { name: second.title })
  expect(container.querySelectorAll('.research-theme-record')).toHaveLength(1)
  ...
  fireEvent.keyDown(screen.getByRole('tab', { name: '当前分析' }), { key: 'End' })
  expect(screen.getByRole('tab', { name: '证据' })).toHaveProperty('ariaSelected', 'true')
  fireEvent.change(screen.getByLabelText('选择主题'), { target: { value: 'theme-credit' } })
  expect(await screen.findByRole('heading', { name: theme().title })).toBeTruthy()
  expect(screen.getByRole('tab', { name: '当前分析' })).toHaveProperty('ariaSelected', 'true')
  expect(window.location.hash).toBe('#research-theme-theme-credit')
  expect(window.location.search).toBe('?tab=research')
  ...
  window.history.replaceState(null, '', '#research-evidence')
  fireEvent(window, new HashChangeEvent('hashchange'))
