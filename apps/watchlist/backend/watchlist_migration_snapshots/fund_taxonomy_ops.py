"""Frozen taxonomy transformations used by historical migrations."""

from __future__ import annotations

from collections.abc import Sequence

from watchlist_migration_snapshots.fund_taxonomy import (
    FUND_TAXONOMY_CODE,
...
def build_taxonomy_context(
    ...
    if path_labels:
        derived_values["fund_regime"] = path_labels[0]
        derived_values["fund_taxonomy_leaf"] = path_labels[-1]
        derived_values["fund_taxonomy_path"] = " / ".join(path_labels)
    for level, label in enumerate(path_labels[1:], start=1):
        if level > FUND_TAXONOMY_MAX_LEVELS:
            break
        derived_values[f"fund_taxonomy_level_{level}"] = label
...
def merge_taxonomy_attributes(
    *,
    ...
    merged = {
        ...
        if key not in FUND_TAXONOMY_DERIVED_KEYS
    }
    derived_values = (
    ...
    if isinstance(derived_values, dict):
        for key, value in derived_values.items():
            if key in FUND_TAXONOMY_DERIVED_KEYS and value is not None:
