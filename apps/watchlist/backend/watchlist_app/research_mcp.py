...
def read_risk_instrument(instrument_id: str,
    ...
    return _risk_instrument_packet(request("context"), instrument_id, section=section,
...
def submit_risk_review(result: SkipValidation[RiskReview]) -> dict:
    """Submit ONE COMPLETE result with summary, priorities and limitations (use [] when there are none). Every retry REPLACES the whole result: preserve all valid fields and fix the reported errors; never send only the missing fields. References must match the retained scope and sources. This saves the result for the running job to finalize; it does not alter RiskCases or PM views. After acceptance acknowledge briefly, without reprinting JSON."""
