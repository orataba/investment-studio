...
from app.reference_data.fund_tag_examples import get_example_fund_attributes
...
from app.services.instrument_resolution import resolve_watchlist_instrument
...
EXAMPLE_ATTRIBUTE_SOURCE_RECORD_ID = "seed/fund-tags-v2"
...
def _seed_example_fund_attributes(session: Session, asset_id: str) -> None:
    example_values = get_example_fund_attributes(asset_id)
    if example_values is None:
        return

    current_values = collapse_latest_attribute_values(
        attribute_repository.get_values_for_asset(session, asset_id)
    )
    updated = False
    for attribute_key, value in example_values.items():
        if attribute_key in current_values:
            continue
        attribute_repository.add_value(
            ...
            value_json=value,
            source_record_id=EXAMPLE_ATTRIBUTE_SOURCE_RECORD_ID,
        )
        current_values[attribute_key] = value
        updated = True
...
def create_watchlist_record(
    ...
    record = watchlist_repository.create(
        session,
        watchlist_id=watchlist_id,
        name=name,
...
def copy_watchlist_record(
    ...
    record = watchlist_repository.duplicate(
        session,
    ...
    try:
        _materialize_watchlist_rows(
            ...
            asset_ids=[item.asset_id for item in source.items],
        )
...
def add_items_to_watchlist(
    ...
    for shared_instrument in resolved_instruments:
        ...
        if detail_asset_id:
            _seed_example_fund_attributes(session, detail_asset_id)
...
def move_items_to_watchlist(
    ...
    return {
        ...
        "already_present_count": max(deleted_count - len(created), 0),
    }
...
def copy_items_to_watchlist(
    ...
    return {
        ...
        "added_count": len(created),
        "already_present_count": max(len(asset_ids) - len(created), 0),
    }
...
def create_view_for_watchlist(
    ...
    _require_watchlist(session, watchlist_id)
    view_id = payload.name.strip().lower().replace(" ", "-")
    if watchlist_repository.get_view(session, watchlist_id=watchlist_id, view_id=view_id) is not None:
    ...
    record = watchlist_repository.create_view(
        session,
        watchlist_id=watchlist_id,
        view_id=view_id,
        name=payload.name,
