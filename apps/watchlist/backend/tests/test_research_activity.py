...
from watchlist_app.services.research_activity import (research_activity, resolve_event_reference,
    judgment_changed_at, judgment_review_receipt, review_receipts)
...
def publish(session, *, events=None, research=None, themes=None, reflection=None):
    ...
    from watchlist_app.services.research_themes import theme_index, save_theme, ThemeInput
    events, research = deepcopy(events or []), deepcopy(research)
    unlinked = [row for row in events if row.get("follow_up", "watch") == "watch" and not row.get("theme_ids")]
    for field in ("questions", "forecasts", "catalysts"):
        unlinked.extend(row for row in (research or {}).get(field, []) if not row.get("theme_id"))
    if unlinked:
        available = theme_index(session, "xlk", active_only=True)
        default = available[0] if available else save_theme(session, "xlk", ThemeInput(title="持续融资研究", question="融资如何影响经营与现金流？"))
        for row in events:
            if row.get("follow_up", "watch") == "watch" and not row.get("theme_ids"):
                row["theme_ids"] = [default["theme_id"]]
        for field in ("questions", "forecasts", "catalysts"):
            for row in (research or {}).get(field, []):
                row.setdefault("theme_id", default["theme_id"])
    ...
    row["themes"] = list(themes or [])
    supplied = {item.get("theme_id") for item in row["themes"]}
    row["themes"].extend({"theme_id": theme["theme_id"], "theme_key": theme["theme_key"]}
        for theme in theme_index(session, "xlk") if theme["status"] == "active" and theme["theme_id"] not in supplied)
...
def test_unpublished_and_foreign_team_notebooks_never_enter_activity(activity_client):
    ...
    with get_session_factory()() as session:
        ...
        session.commit()
        assert not any(row["kind"] != "theme" for row in research_activity(session, "xlk")["updates"])
...
def test_ongoing_work_lives_in_themes_and_brief_events_stay_separate(activity_client):
    with get_session_factory()() as session:
    ...
    assert "current_followups" not in data
    assert len(data["recent_events"]) == 1 and data["recent_events"][0]["event_key"] == "brief"
    theme = activity_client.get("/api/research/instruments/xlk/themes").json()["themes"][0]
    assert len(theme["current_questions"]) == 1
    assert theme["current_questions"][0]["last_reviewed_at"] is None
    assert {row["kind"] for row in theme["updates"]} == {"theme", "event", "question"}
    assert len(data["updates"]) == 4


def test_theme_groups_event_questions_and_hides_finished_work(activity_client):
    with get_session_factory()() as session:
        ...
        assert len(research_activity(session, "xlk")["updates"]) == 5
...
def test_supported_questions_keep_tracking_and_explicit_lifecycle_preserves_history(activity_client):
    ...
    with get_session_factory()() as session:
        ...
        assert themes_view(session, "xlk")["themes"][0]["current_questions"][0]["update_id"] == original["update_id"]
        ...
        assert not themes_view(session, "xlk")["themes"][0]["current_questions"]
...
def test_legacy_supported_question_stays_active_without_rewriting_its_history(activity_client):
    ...
    with get_session_factory()() as session:
        ...
        session.commit()
        before = research_activity(session, "xlk", include_recent_events=True)
        assert len(themes_view(session, "xlk")["themes"][0]["current_questions"]) == 1
        assert len(read_dossier(session, "xlk")["review_agenda"]["tracked_questions"]) == 1
        publish(session)
        assert research_activity(session, "xlk", include_recent_events=True) == before
...
def _reviewed_event(session):
    row = next(row for row in research_activity(session, "xlk")["updates"] if row["kind"] == "event" and not row.get("superseded"))
    return {**row, "last_changed_at": judgment_changed_at(row), "last_reviewed_at": None,
            **judgment_review_receipt(row, review_receipts(session, "xlk"))}


def test_exact_receipt_advances_check_clock_without_inventing_progress(activity_client):
    with get_session_factory()() as session:
        publish(session, events=[event()])
        before = _reviewed_event(session)
        run = publish(session, reflection={"status": "insufficient_evidence", "summary": "原文尚未更新",
            "reviewed_update_ids": [before["update_id"]]})
        after = _reviewed_event(session)
        ...
        changed = _reviewed_event(session)
...
def test_active_themes_own_work_but_paused_dedicated_questions_stay_paused(activity_client):
    ...
    with get_session_factory()() as session:
        ...
        assert "current_followups" not in research_activity(session, "xlk", include_recent_events=True)
        from watchlist_app.services.research_themes import themes_view
    ...
    data = activity_client.get("/api/research/instruments/xlk/activity").json()
    assert "current_followups" not in data
    assert activity_client.get("/api/research/instruments/xlk/themes").json()["themes"][0]["current_questions"] == []
...
def test_event_receipt_does_not_check_its_grouped_question_or_theme_judgment(activity_client):
    ...
    with get_session_factory()() as session:
        ...
        current = _reviewed_event(session)
        child = next(row for row in themes_view(session, "xlk")["themes"][0]["current_questions"] if row["title"] == "资金如何使用")
        reviewed = publish(session, reflection={"status": "reviewed", "summary": "只复核了原事件条款",
            "reviewed_update_ids": [current["update_id"]]})
        after = _reviewed_event(session)
        ...
        assert _reviewed_event(session)["last_reviewed_at"] == after["last_reviewed_at"]
