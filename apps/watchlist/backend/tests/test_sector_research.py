...
def test_failed_search_cannot_become_no_event_success(client, monkeypatch):
    seed_sector(client, monkeypatch)
    with get_session_factory()() as session:
        run,_=service.begin_run(session,["xlk"])
        ...
        assert run.context_json["reviews"]["xlk"]["summary"]=="本次未完成公开资讯检索。"
        assert any("未成功" in s for s in run.context_json["reviews"]["xlk"]["coverage"])
