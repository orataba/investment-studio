...
XLB、XLC、XLE、XLF、XLI、XLK、XLP、XLRE、XLU、XLV、XLY 的 FMP 数据由 Investment Studio 自己采集和维护。完整成份、公司资料、年度/季度预期与最近价格逐次保存在共享 PostgreSQL 的 `instrument_data.instrument_reference_observation`，当前投影使用 `instrument_reference_snapshot.value_json.sections.sector_market_data`，附着于 ETF 登记记录；成份公司不因此进入观察列表。Watchlist 只读取本项目留存快照，研究按截止时点绑定已采集观察；采集历史独立于模型是否运行或成功完成，缺失时明确标注缺口。普通标的概况接口不返回这份较大的明细，研究运行按需读取并绑定到本轮证据。

成份公司预期按每轮 FMP 采集快照留存，并按相同公司、预测财期、年度/季度、指标和币种比较。财期滚动及覆盖增减不视为上修或下修；原始预期未提供币种时依据同公司最近财报的 reportedCurrency 推断，保留财报日期、来源和推断状态；若仍未知则仅保留待核实读数，不能用报价币种替代。可比变化会进入每日分析和独立核证，只有重要变化才形成事件。采集范围支持上述 11 只美股行业 ETF，其他适用 ETF 明确显示未接入，已披露为商品资产的 ETF 不展示公司盈利预期入口。
