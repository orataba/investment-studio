...
def _wait_for_report(base, result, headers):
    deadline = time.monotonic() + 3900
    while result["status"] in {"queued", "running"}:
        if time.monotonic() >= deadline:
            raise SystemExit(f"Could not confirm briefing completion within 65 minutes; inspect retained report {result['report_id']}.")
        time.sleep(5)
        try:
            ...
            if error.code not in {502, 503, 504}:
                raise
            print(f"Briefing status temporarily unavailable (HTTP {error.code}); continuing to monitor report {result['report_id']}.", file=sys.stderr)
        except (URLError, TimeoutError, ConnectionError):
            print(f"Briefing status connection unavailable; continuing to monitor report {result['report_id']}.", file=sys.stderr)
    return result
...
def main():
    ...
    with urlopen(req, timeout=30) as response:
        result = json.load(response)
    if args.scheduled:
        result = _wait_for_report(base, result, headers)
