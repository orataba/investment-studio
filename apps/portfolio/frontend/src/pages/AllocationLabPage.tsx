...
import {
  createPortfolioAllocationResearchRun,
  getPortfolioPolicyReplayBenchmarkComparison,
  getPortfolioInstruments,
  getPortfolios,
  getPortfolioRiskPolicy,
  getPortfolioTaxonomyCatalog,
  getPortfolioAllocationResearchWorkbench,
  updatePortfolioAllocationResearchSettings,
  type PortfolioPolicyReplayBenchmarkComparisonResponse,
  type PortfolioPolicyReplayBenchmarkRecord,
  type PortfolioPolicyReplayMetricsRecord,
  type PortfolioPolicyReplayPointRecord,
  type PortfolioPolicyReplayRebalanceFrequency,
  type PortfolioPolicyReplayRelativeMetricsRecord,
  type PortfolioPolicyReplaySleevePointRecord,
  type PortfolioAllocationResearchAsOfMode,
  type PortfolioAllocationResearchCapitalMode,
  type PortfolioAllocationResearchPlanningScopeOption,
  type PortfolioAllocationResearchRunRecord,
  type PortfolioAllocationResearchTopSleeveWeightBoundRecord,
  type PortfolioAllocationResearchWorkbenchResponse,
...
const REBALANCE_OPTIONS: Array<{ value: PortfolioPolicyReplayRebalanceFrequency; label: string }> = [
  { value: '1w', label: '1W' },
  { value: '1m', label: '1M' },
  { value: '3m', label: '3M' },
]

type AllocationResearchRunSetupDraft = {
  asOfMode: PortfolioAllocationResearchAsOfMode
  asOfDate: string
  capitalMode: PortfolioAllocationResearchCapitalMode
  grossExposure: string
  targetVolatilityPct: string
  maxGrossExposure: string
  frozenNodeIds: string[]
  topSleeveBounds: AllocationResearchTopSleeveBoundDraft[]
  policyReplayRebalanceFrequency: PortfolioPolicyReplayRebalanceFrequency
  benchmarkInstrumentId: string
}

type AllocationResearchTopSleeveBoundDraft = {
...
function isVolatilityCapitalMode(value: PortfolioAllocationResearchCapitalMode) {
...
function buildPlanningScopeOptions(
...
): PortfolioAllocationResearchPlanningScopeOption[] {
  ...
  const options: PortfolioAllocationResearchPlanningScopeOption[] = [
    {
...
function PolicyReplayLineChart({
  points,
  benchmarkPoints = [],
  benchmarkLabel = null,
}: {
  points: PortfolioPolicyReplayPointRecord[]
  benchmarkPoints?: PortfolioPolicyReplayPointRecord[]
  ...
  if (visiblePoints.length < 2) {
    return <div className="empty-state">No policy replay series.</div>
  }
  const width = 720
  const height = 250
  const padding = 36
  ...
  return (
    <div className="research-chart">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="research-chart-svg" role="img" aria-label="Policy replay curve">
...
function sleeveSeries(points: PortfolioPolicyReplaySleevePointRecord[]) {
...
function PolicyReplaySleeveLineChart({
  points,
  ariaLabel,
}: {
  points: PortfolioPolicyReplaySleevePointRecord[]
  ariaLabel: string
}) {
...
function PolicyReplaySleeveStackedAreaChart({
  points,
}: {
  points: PortfolioPolicyReplaySleevePointRecord[]
}) {
...
function MetricTable({
  ...
  run: PortfolioAllocationResearchRunRecord
  benchmark: PortfolioPolicyReplayBenchmarkRecord | null
  relativeMetrics: PortfolioPolicyReplayRelativeMetricsRecord | null
}) {
  const metrics = run.detail?.policy_replay?.metrics ?? null
  const benchmarkMetrics = benchmark?.metrics ?? null
  const relative = relativeMetrics

  function periodLabel(record: PortfolioPolicyReplayMetricsRecord | PortfolioPolicyReplayRelativeMetricsRecord | null) {
  ...
  function formatMetricCell(
    ...
    record: PortfolioPolicyReplayMetricsRecord | PortfolioPolicyReplayRelativeMetricsRecord | null,
  ) {
    if (
...
export default function AllocationLabPage() {
  ...
  const [operatingProfile, setOperatingProfile] = useState<PortfolioOperatingProfile | null>(null)
  const [workbench, setWorkbench] = useState<PortfolioAllocationResearchWorkbenchResponse | null>(null)
  ...
  const [dynamicScopeOptions, setDynamicScopeOptions] = useState<PortfolioAllocationResearchPlanningScopeOption[] | null>(null)
  ...
  const [benchmarkComparison, setBenchmarkComparison] =
    useState<PortfolioPolicyReplayBenchmarkComparisonResponse | null>(null)
  ...
  const runSetupDraftRef = useRef<AllocationResearchRunSetupDraft>({
    asOfMode: 'dynamic',
    asOfDate: '',
    ...
    frozenNodeIds: [],
    topSleeveBounds: [],
    policyReplayRebalanceFrequency: '1m',
    benchmarkInstrumentId: '',
  })

  const [planningTaxonomyId, setPlanningTaxonomyId] = useState('')
  const [asOfMode, setAsOfMode] = useState<PortfolioAllocationResearchAsOfMode>('dynamic')
  const [asOfDate, setAsOfDate] = useState('')
  const [capitalMode, setCapitalMode] = useState<PortfolioAllocationResearchCapitalMode>('unit_notional')
  ...
  const [topSleeveBounds, setTopSleeveBounds] = useState<AllocationResearchTopSleeveBoundDraft[]>([])
  const [policyReplayRebalanceFrequency, setPolicyReplayRebalanceFrequency] =
    useState<PortfolioPolicyReplayRebalanceFrequency>('1m')
  ...
  async function reloadWorkbench(targetPortfolioId = portfolioId) {
    ...
    try {
      ...
      const portfolio = portfolios.find((item) => item.portfolio_id === targetPortfolioId)
      if (
        !isRequestCurrent(workbenchRequestSequenceRef, request, currentPortfolioIdRef.current)
      ) {
        return
      }
      if (!portfolio) {
      ...
      setOperatingProfile(portfolio.operating_profile)
      if (portfolio.operating_profile === 'external_etf_rotation') {
        setWorkbench(null)
        setWorkspaceError(null)
        return
      }
      const response = await getPortfolioAllocationResearchWorkbench(targetPortfolioId)
      if (
        !isRequestCurrent(
  ...
  useEffect(() => {
    if (!portfolioId || operatingProfile !== 'standard_taxonomy') {
  ...
  useEffect(() => {
    ...
    const nextAsOf = resolveAllocationResearchAsOfDraft(workbench.settings, workbench.as_of_date)
    const nextBenchmarkInstrumentId = workbench.settings.policy_replay_benchmark_instrument_id ?? ''
    ...
    const nextDraft: AllocationResearchRunSetupDraft = {
      ...nextAsOf,
      ...
      policyReplayRebalanceFrequency: workbench.settings.policy_replay_rebalance_frequency ?? '1m',
    ...
    setPolicyReplayRebalanceFrequency(nextDraft.policyReplayRebalanceFrequency)
  ...
  useEffect(() => {
    const allocationResearchRunId = latestRun?.allocation_research_run_id ?? ''
    const selectedBenchmarkId = benchmarkInstrumentId.trim()
    const storedBenchmarkId = latestRun?.detail?.policy_replay_benchmark?.instrument_id ?? ''
    if (!portfolioId || !allocationResearchRunId || latestRun?.status !== 'completed' || !selectedBenchmarkId) {
      setBenchmarkComparison(null)
      setBenchmarkComparisonLoading(false)
      setBenchmarkComparisonError(null)
      return undefined
    }
    if (selectedBenchmarkId === storedBenchmarkId && latestRun.detail?.policy_replay_benchmark) {
      setBenchmarkComparison(null)
      setBenchmarkComparisonLoading(false)
      setBenchmarkComparisonError(null)
      return undefined
    ...
    setBenchmarkComparisonError(null)
    getPortfolioPolicyReplayBenchmarkComparison(portfolioId, allocationResearchRunId, selectedBenchmarkId)
      .then((response) => {
        if (!cancelled) {
    ...
    benchmarkInstrumentId,
    latestRun?.detail?.policy_replay_benchmark,
    latestRun?.allocation_research_run_id,
    latestRun?.status,
    portfolioId,
  ])

  const selectedScopeOptions = useMemo<PortfolioAllocationResearchPlanningScopeOption[]>(() => {
  ...
  function scheduleAutoSave(nextDraft: AllocationResearchRunSetupDraft) {
  ...
  function updateRunSetupDraft(updates: Partial<AllocationResearchRunSetupDraft>) {
    const nextDraft = {
      ...runSetupDraftRef.current,
      ...updates,
    }
    ...
    function updateTopSleeveBound(nodeId: string, field: 'minWeightPct' | 'maxWeightPct', value: string) {
      setTopSleeveBounds((current) => {
        ...
        const nextItem: AllocationResearchTopSleeveBoundDraft = {
    ...
    function serializeTopSleeveBounds(
      draftBounds: AllocationResearchTopSleeveBoundDraft[],
    ): PortfolioAllocationResearchTopSleeveWeightBoundRecord[] {
  ...
  async function persistSettings({
    ...
    draft?: AllocationResearchRunSetupDraft
    ...
    const allocationResearchAsOf = serializeAllocationResearchAsOf(draft)
    if (allocationResearchAsOf.as_of_mode === 'pinned' && !allocationResearchAsOf.as_of_date) {
      throw new Error('Select a pinned analysis date.')
    }
    return updatePortfolioAllocationResearchSettings(targetPortfolioId, {
      planning_taxonomy_id: planningTaxonomyId || null,
      comparator_taxonomy_node_id: null,
      ...allocationResearchAsOf,
      ...
      policy_replay_rebalance_frequency: draft.policyReplayRebalanceFrequency,
      policy_replay_benchmark_instrument_id: draft.benchmarkInstrumentId || null,
      notes: null,
    })
  }

  async function handleRunAllocationResearch() {
    ...
    try {
      await autoSaveQueueRef.current.enqueue(async () => {
        ...
        await createPortfolioAllocationResearchRun(targetPortfolioId, { requested_by: 'workspace-ui' })
      })
      if (!isRequestCurrent(runRequestSequenceRef, runRequest, currentPortfolioIdRef.current)) {
        return
      }
      setNotice('Allocation research run completed.')
  ...
  const policyReplay = latestRun?.detail?.policy_replay ?? null
  ...
  const storedBenchmark = latestRun?.detail?.policy_replay_benchmark ?? null
  const selectedBenchmarkId = benchmarkInstrumentId.trim()
  const displayBenchmark = selectedBenchmarkId
    ? benchmarkComparison?.policy_replay_benchmark ??
      (selectedBenchmarkId === (storedBenchmark?.instrument_id ?? '') ? storedBenchmark : null)
    : null
  const displayRelativeMetrics = selectedBenchmarkId
    ? benchmarkComparison?.policy_replay_relative_metrics ??
      (selectedBenchmarkId === (storedBenchmark?.instrument_id ?? '')
        ? latestRun?.detail?.policy_replay_relative_metrics ?? null
        : null)
    : null

  return (
    <PortfolioWorkspaceLayout activeSection="Allocation Lab" toolbarLabel="View: Allocation Lab">
      ...
      {!loading && operatingProfile === 'external_etf_rotation' ? (
        <section className="panel">
          <div className="empty-state">
            Allocation Lab is not applicable to portfolios managed by an external ETF rotation strategy.
          </div>
        </section>
      ) : null}

      {!loading && operatingProfile === 'standard_taxonomy' && !workbench && !workspaceError ? (
      ...
      {workbench ? (
        <>
              ...
              <fieldset className="research-settings-fieldset" disabled={actionPending === 'run'}>
                ...
                <div className="taxonomy-form-grid taxonomy-form-grid-wide research-settings-grid">
                  <label>
                    ...
                    <select
                      value={asOfMode}
                      onChange={(event) => {
                        const nextMode = event.target.value as PortfolioAllocationResearchAsOfMode
                        const nextDate =
                          nextMode === 'dynamic'
                  ...
                  <label>
                    ...
                    <select
                      value={capitalMode}
                      onChange={(event) => {
                        const nextCapitalMode = event.target.value as PortfolioAllocationResearchCapitalMode
                        const nextDraftUpdates: Partial<AllocationResearchRunSetupDraft> = { capitalMode: nextCapitalMode }
                  ...
                  <div className="research-freeze-field" ref={frozenMenuRef}>
                    ...
                    <label>
                      <span>Rebalance</span>
                    <select
                      value={policyReplayRebalanceFrequency}
                      onChange={(event) => {
                        const nextFrequency = event.target.value as PortfolioPolicyReplayRebalanceFrequency
                        setPolicyReplayRebalanceFrequency(nextFrequency)
                        updateRunSetupDraft({ policyReplayRebalanceFrequency: nextFrequency })
                      }}
                    >
                ...
                <div className="research-run-action-field">
                  <button
                    ...
                    onClick={() => void handleRunAllocationResearch()}
          ...
          {!latestRun ? (
            ...
            <section className="panel">
              <div className="inline-notice inline-notice-error">{latestRun.error_message ?? 'Allocation research run failed.'}</div>
            </section>
          ) : (
            <>
              {staleRun ? (
                <section className="panel">
                  <div className="inline-notice inline-notice-warning">
                    ...
                    Run Allocation Research again before using these weights for allocation or orders.
              ...
              <section className="performance-block-grid allocation-policy-replay-grid">
                <div className="performance-section-block research-chart-panel">
                  <div className="panel-header panel-header-inline">
                    <div><div className="panel-title">Policy Replay</div></div>
                  </div>
                  <PolicyReplayLineChart
                    points={policyReplay?.points ?? []}
              ...
              <section className="performance-block-grid research-sleeve-grid">
                <div className="performance-section-block research-chart-panel">
                  ...
                  <PolicyReplaySleeveStackedAreaChart points={policyReplay?.top_sleeve_weight_points ?? []} />
                </div>
                <div className="performance-section-block research-chart-panel">
                  <div className="panel-header panel-header-inline">
                    <div><div className="panel-title">Top-Level Sleeve Contribution</div></div>
                  </div>
                  <PolicyReplaySleeveLineChart
                    points={policyReplay?.top_sleeve_contribution_points ?? []}
