  ...
  return <section className="portfolio-section-block concentration-panel" aria-label={text('Concentration', '集中度')}>
    <div className="portfolio-detail-toolbar portfolio-section-toolbar risk-section-toolbar">
      <div>
        <div className="panel-title portfolio-title-with-hint"><span>{text('Concentration', '集中度')}</span>
          <InfoHint label={text('Concentration basis', '集中度口径')} detail={text('Direct securities use gross market exposure across accounts. FCNs use remaining nominal principal, allocated once across underlying securities for taxonomy grouping. Equal allocation is a management convention, not equal risk. Options and cash are excluded from exposure; NAV is unchanged. These are not risk contributions or potential stock delivery exposures.', '直接证券按账户汇总绝对市值敞口；FCN 按剩余名义本金，在挂钩标的之间分配后归入分类，不重复计算本金。等分属于管理约定，不代表风险均分。期权和现金不进入敞口分子，NAV 不变。这些占比不代表风险贡献或潜在接票股票敞口。')} />
        </div>
    ...
    {data && !error ? <>
      ...
      {visibleRows.length ? <HorizontalTableScroll className="concentration-table-wrap"><table className="concentration-table" aria-label={text('Concentration exposures and limits', '集中度敞口及限额')}>
        <thead><tr><th>{text('Member', '成员')}</th><th>{text('Exposure', '敞口分布')}</th><th>{text('Exposure / NAV', '敞口 / NAV')}</th><th>{text('Limit / NAV', '上限 / NAV')}</th><th>{text('Headroom', '剩余额度')}</th><th>{text('Status', '状态')}</th></tr></thead>
