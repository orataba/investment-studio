...
function row(id: string, parent: ResearchSolutionTreeRow | null, kind: ResearchSolutionTreeRow['row_kind'], overrides: Partial<ResearchSolutionTreeRow> = {}): ResearchSolutionTreeRow {
  return { row_id: id, parent_row_id: parent?.row_id ?? null, row_kind: kind, member_type: kind, member_id: id, label: id,
    ...
    current_value_base: 100, current_exposure_base: 150, current_exposure_weight: .15, target_value_base: 120, target_weight: .12,
    rebalance_value_base: 20, exposure_status: 'complete', trade_constraint: 'adjustable', risk_model_status: 'modeled',
...
const cash = row('Cash', root, 'cash', { target_risk_share: null, solved_risk_share: null, current_exposure_base: null, current_exposure_weight: null, exposure_status: 'not_applicable', rebalance_value_base: 10 })
const derivatives = row('Derivatives', root, 'derivatives', { current_value_base: -50, target_value_base: -50, target_weight: -.05, target_risk_share: null, solved_risk_share: null, current_exposure_base: null, current_exposure_weight: null, exposure_status: 'unavailable', trade_constraint: 'no_trade', execution_status: 'no_trade', risk_model_status: 'excluded', rebalance_value_base: 0 })
const tree: ResearchSolutionTreeData = { schema_version: 1, as_of_date: '2026-06-30', base_currency: 'USD', portfolio_nav: 1000,
...
describe('Research solution tree', () => {
  it('shows the saved full hierarchy, report currency, signed trades and no-trade semantics', () => {
    ...
    expect(within(instrument).getByText('Buy')).toBeInTheDocument()
    expect(within(screen.getByText('Second instrument').closest('tr')!).getByText('−$30.00')).toBeInTheDocument()
    expect(within(screen.getByText('Cash').closest('tr')!).getByText('Cash change')).toBeInTheDocument()
    ...
    expect(within(derivative).getAllByText('No trade')).toHaveLength(2)
  ...
  it('collapses to first-level categories and exports all hidden descendants as numbers', async () => {
    ...
    expect(instrument[6]).toBe(15)
    expect(instrument[8]).toBe(20)
    expect(instrument[14]).toBe('2026-06-30')
    expect(instrument[15]).toBe('USD')
    expect(instrument[20]).toBe('stale / constrained_solution')
  ...
  it('discloses archived scope and risk basis instead of claiming a new global NAV calculation', async () => {
    ...
    renderTree(legacy)
    await user.click(screen.getByRole('button', { name: /^Capital weight basis:/ }))
    expect(screen.getByRole('tooltip')).toHaveTextContent('Weights retain the saved research-scope basis without recalculation')
    await user.click(screen.getByRole('button', { name: /^Risk contribution basis:/ }))
    expect(screen.getByRole('tooltip')).toHaveTextContent('Local targets may differ in scope')
    const exported = solutionTreeExportRows(legacy.detail.solution_tree, 'legacy', 'stale', false)
    expect(exported[0][7]).toBe('Target capital weight / Saved scope (%)')
  })
  ...
  it('writes a real OOXML workbook with numeric amount and percentage cells', () => {
    ...
    expect(xml).toContain('<c r="G5"><v>15</v></c>')
