...
describe('FundDistributionTasksPanel', () => {
  ...
  it('shows distribution facts to readers without allowing review or reconciliation', async () => {
    ...
    await userEvent.click(await screen.findByRole('button', { name: 'Review distributions (1)' }))
  ...
  it.each([['Record Cash Dividend', 'dividend'], ['Record Reinvestment', 'dividend_reinvestment']] as const)('shows confirmed distribution facts and delegates %s', async (buttonLabel, transactionType) => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Review distributions (1)' }))
    expect(await screen.findByText('Example Private Fund')).toBeInTheDocument()
    ...
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Review distributions (1)' })).toHaveFocus()
  })
  ...
  it('shows the loading error in the shared hint and lets readers retry from the review', async () => {
    accessState.can_edit = false
    ...
    expect(trigger).toHaveClass('investment-studio-info-hint-warning')
    expect(trigger).toHaveTextContent('!')
  ...
  it('uses the same hint for hover and click without opening the review', async () => {
    const user = userEvent.setup()
    render(<FundDistributionTasksPanel portfolioId="portfolio-1" accountNames={{}} refreshKey={0} onRecord={vi.fn()} />)
    const hint = await screen.findByRole('button', { name: /Fund distribution reviews: 1 need attention/ })
    await user.hover(hint)
    expect(await screen.findByRole('tooltip')).toHaveTextContent('Example Private Fund')
    await user.click(hint)
    expect(screen.getByRole('tooltip')).toHaveTextContent('1 need attention')
    expect(screen.queryByRole('region', { name: 'Fund distribution reviews' })).not.toBeInTheDocument()
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
    expect(apiMocks.reconcilePortfolioInstrumentEventTasks).not.toHaveBeenCalled()
  })

  it('expands the review inline and closes with Escape or the toggle', async () => {
    const user = userEvent.setup()
    render(<FundDistributionTasksPanel portfolioId="portfolio-1" accountNames={{}} refreshKey={0} onRecord={vi.fn()} />)
    const trigger = await screen.findByRole('button', { name: 'Review distributions (1)' })
    await user.click(trigger)
    const region = screen.getByRole('region', { name: 'Fund distribution reviews' })
    expect(region).toHaveClass('fund-distribution-review-panel')
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
    await user.click(screen.getByRole('textbox', { name: 'Operator identity' }))
    await user.keyboard('{Escape}')
    expect(screen.queryByRole('region', { name: 'Fund distribution reviews' })).not.toBeInTheDocument()
    await waitFor(() => expect(trigger).toHaveFocus())
    await user.click(trigger)
    expect(screen.getByRole('region', { name: 'Fund distribution reviews' })).toBeInTheDocument()
    await user.click(trigger)
    expect(screen.queryByRole('region', { name: 'Fund distribution reviews' })).not.toBeInTheDocument()
  })
