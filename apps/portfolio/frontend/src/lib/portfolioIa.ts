...
export const workspacePrimaryNavigation = [
  ...
  { label: 'Allocation Lab', href: '/allocation-research' },
] as const
...
export const portfolioPageDefinitions: Record<string, PortfolioPageDefinition> = {
  ...
  risk: {
    ...
    coreBlocks: [
      ...
      { title: 'Allocation Policy Drift', owner: 'Explicit SAA/TAA TargetSet comparators', role: 'Compare actual weights and risk shares against SAA/TAA target dimensions without cross-source substitution' },
  ...
  transactions: {
    ...
    href: '/transactions',
    family: 'Ledger',
    toolbarLabel: 'Page: Transaction Facts',
    summary:
      'Transactions owns the editable portfolio facts for buys, sells, cash flows, fees, taxes, and transfers; published calculations remain separate read models.',
    ...
    coreBlocks: [
      { title: 'Transaction fact table', owner: 'Transaction', role: 'Show canonical facts with sort and filter support' },
      { title: 'Filters by account / instrument / type / period', owner: 'Transaction workspace', role: 'Slice source facts without changing page ownership' },
      { title: 'Direct edit / delete support', owner: 'Transaction editor', role: 'Maintain correct source facts and recalc triggers' },
      { title: 'Cash flow visibility', owner: 'Transaction facts', role: 'Keep external and internal cash movement explicit' },
    ],
  ...
  accounts: {
    ...
    summary:
      'Accounts combines editable custody configuration with the current immutable Portfolio Daily account statement for cash, accruals, positions, and cost basis.',
    primaryQuestion: '底层账户结构和归属是什么？',
    primaryObjects: ['Account', 'published balance components', 'published positions', 'linked transactions'],
    sharedContext: [
      'portfolio_id: current workspace portfolio',
      'base_currency: portfolio base currency',
    ...
    coreBlocks: [
      ...
      { title: 'Published balance components', owner: 'Portfolio Daily publication', role: 'Expose exact cash, receivable, payable, and accrual state' },
    ...
    comparator: ['No default benchmark comparator; this page is custody structure and published state'],
  ...
  allocationResearch: {
    key: 'allocation_research',
    label: 'Allocation Lab',
    href: '/allocation-research',
    family: 'Period',
    toolbarLabel: 'Page: Allocation Lab',
    summary:
      'Allocation Research is the portfolio workbench for current target-weight solve setup, current context, run history, and artifacts.',
    primaryQuestion: '当前组合要按什么 planning 语境做研究，已经跑过哪些结果，下一步该 handoff 什么？',
    primaryObjects: ['AllocationResearchSettings', 'AllocationResearchRun', 'AllocationResearchArtifact'],
    sharedContext: ['portfolio_id: current workspace portfolio', 'current default planning taxonomy', 'current portfolio facts'],
    coreBlocks: [
      { title: 'Run setup', owner: 'AllocationResearchSettings', role: 'Select planning taxonomy, as_of_date, and lookback window for the next run' },
      { title: 'Current context', owner: 'Portfolio facts + planning context', role: 'Show the snapshot the next run will operate on' },
      { title: 'Run list', owner: 'AllocationResearchRun', role: 'List allocation research runs in portfolio context' },
      { title: 'Policy Replay', owner: 'AllocationPolicyReplay', role: 'Replay allocation policies without presenting hypothetical results as live performance' },
      { title: 'Artifact viewer', owner: 'Allocation research artifacts', role: 'Display run output without leaving the portfolio workspace' },
      { title: 'Handoff to portfolio context', owner: 'AllocationResearchRun handoff', role: 'Promote allocation research output into portfolio analysis context' },
    ],
    comparator: ['No canonical comparator; this page is workflow-oriented'],
    conditionalBlocks: ['Unavailable for the external_etf_rotation operating profile', 'Method templates may grow later, but this page should not become a raw backend schema editor'],
    drillDowns: [{ source: 'Allocation research artifact', target: 'Portfolio discussion context' }],
  ...
  taxonomies: {
    ...
    drillDowns: [{ source: 'Planning target set', target: 'Risk and Allocation Research target resolution' }],
