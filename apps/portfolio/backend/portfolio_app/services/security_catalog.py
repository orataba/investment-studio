...
The child command loads its own data environment; Portfolio does not own the
...
def _run_command(arguments: list[str], *, payload: dict | None = None) -> dict:
    command = Path(__file__).resolve().parents[5] / "bin" / "investment-studio"
    try:
        result = subprocess.run(
            [str(command), "data", "securities", *arguments],
            input=json.dumps(payload) if payload is not None else None,
            # Runtime loaders preserve caller values. Do not let Portfolio's
