...
from app.services.ledger import build_position_lots, derive_ledger_postings
...
def _account_cost_methods(accounts: list[dict[str, object]]) -> dict[str, str]:
    return {
        str(account.get("account_id") or ""): str(account.get("cost_basis_method") or "moving_average")
...
def build_daily_portfolio_snapshots(
    ...
    *,
    ...
    for as_of_date in _iter_dates(resolved_start_date, resolved_end_date):
        ...
        if not cash_complete or not cost_basis_complete or not position_valuation_complete:
        ...
        if (
            ...
            nav = resolved_cash_balance + resolved_position_market_value
...
def _build_contribution_group_end_states(
    *,
    ...
    open_position_lots = [position_lot for position_lot in position_lots if position_lot.get("status") == "open"]
    ...
    for state in states.values():
        ...
        if axis == "account":
            state["ending_value_base"] = (
                cash_balance_base + position_market_value_base
                if cash_balance_base is not None and position_market_value_base is not None
                else None
            )
        else:
    ...
    *,
    ...
    for as_of_date in _iter_dates(resolved_start_date, resolved_end_date):
        ...
        for candidate_group_key in group_keys:
            ...
            if previous_state is None:
                ...
                asset_currency_gains, asset_fx_stale = _compute_currency_translation_gain(
                ...
                if axis == "account":
                    cash_currency_gains, cash_fx_stale = _compute_currency_translation_gain(
    ...
    if in_period_snapshots:
        ...
        if any(str(snapshot.get("coverage_state") or "unavailable") != "complete" for snapshot in in_period_snapshots):
            coverage_state = "partial"
        if any(str(item.get("coverage_state") or "unavailable") != "complete" for item in daily_slices):
            coverage_state = "partial"
