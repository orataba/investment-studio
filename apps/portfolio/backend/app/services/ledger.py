...
def _resolve_cost_basis_method(account_cost_methods: dict[str, str] | None, account_id: str) -> str:
    resolved = str((account_cost_methods or {}).get(account_id) or "moving_average")
    return resolved if resolved in {"moving_average", "fifo"} else "moving_average"
...
def build_position_lots(
    ...
    *,
    ...
    for raw_position_lot in all_position_lots:
        ...
        opened_at_value = _parse_iso_date(raw_position_lot.get("opened_at"))
        closed_at_value = _parse_iso_date(raw_position_lot.get("closed_at")) or resolved_as_of_date
        holding_period_days = None
        if opened_at_value is not None and closed_at_value is not None:
            holding_period_days = max((closed_at_value - opened_at_value).days, 0)
...
def build_account_workspace(
    ...
    *,
    ...
    position_lots = build_position_lots(portfolio_id, accounts, transactions, status="open")
    ...
    for bucket in positions_by_account_asset.values():
        ...
        if market_value is not None:
            converted_market_value = convert_amount(
                market_value,
    ...
    for account in accounts:
        ...
        derived_cash_balance_base = convert_amount(
...
def list_ledger_postings(
    ...
    *,
    ...
    for posting in postings:
        ...
        trade_date_value = str(posting.get("trade_date") or "")
        if start_date and trade_date_value < start_date.isoformat():
            continue
        if end_date and trade_date_value > end_date.isoformat():
            continue
