...
import app.services.performance as performance_service
...
def _candidate_quote_bases(detail: dict[str, object]) -> list[str]:
    ...
    if isinstance(policy, dict):
        for role in ("valuation", "reference", "chart"):
...
def _resolve_dimension_target_rows(
    ...
    *,
    ...
    if selected_dimension == TARGET_DIMENSION_SCOPE_DEFAULT:
        default_dimension = (
            str(state.node_by_id.get(scope_node_id, {}).get("default_target_dimension") or TARGET_DIMENSION_WEIGHT)
            if scope_node_id
