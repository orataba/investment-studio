...
class AssetIdentifierContract(BaseModel):
    identifier_type: IdentifierType
    identifier_value: str = Field(min_length=1)
    is_primary: bool = False


class AssetCoreContract(BaseModel):
    asset_id: str = Field(min_length=1)
    asset_name: str = Field(min_length=1)
    asset_type: AssetType
    currency: str = Field(min_length=1, max_length=8)
    identifiers: list[AssetIdentifierContract] = Field(default_factory=list)
...
class AccountCreateRequest(BaseModel):
    ...
    def validate_account_contract(self) -> "AccountCreateRequest":
        ...
        if self.account_type == "securities_account" and self.cost_basis_method is None:
            self.cost_basis_method = "moving_average"
        if self.allowed_asset_types:
...
class TaxonomyCreateRequest(BaseModel):
    ...
    @field_validator("name", "taxonomy_type", "status", mode="before")
    @classmethod
