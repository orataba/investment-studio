...
from app.services.instrument_registry import InstrumentRegistryError, get_registry_instrument
...
def _list_transactions_as_of_trade_date(portfolio_id: str, trade_date: date) -> list[dict[str, object]]:
    return list_transactions(portfolio_id, end_date=trade_date)
...
def _list_transactions_as_of_trade_moment(
    ...
    *,
    ...
    return [
        record
        for record in _list_transactions_as_of_trade_date(portfolio_id, trade_date)
...
def _list_transactions_as_of_entitlement_moment(
    ...
    *,
    ...
    if entitlement_date < trade_date:
        return _list_transactions_as_of_trade_date(portfolio_id, entitlement_date)
...
def create_transaction_record(
    ...
    account_cost_methods = {
        str(account_item.get("account_id") or ""): str(account_item.get("cost_basis_method") or "moving_average")
    ...
    account_lookup = {item["account_id"]: item for item in list_accounts(portfolio_id)}
    return _serialize_transaction(portfolio_id, record, account_lookup)
