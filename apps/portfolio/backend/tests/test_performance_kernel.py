...
def _test_instrument_detail(
    *,
    ...
    if instrument_type == "public_fund":
...
def test_period_calculation_groups_support_instrument_type_axis(client, monkeypatch):
    ...
    for index, (instrument_id, instrument_name, instrument_type, amount) in enumerate(
        [
            ...
            ("fund-us-test", "Test Fund", "public_fund", 200.0),
        ],
        start=2,
    ):
        transactions.append(
    ...
    assert groups["public_fund"]["group_label"] == "Public Fund"
    assert isclose(groups["public_fund"]["final_value"], 190.0, rel_tol=0.0, abs_tol=1e-12)
    assert isclose(groups["public_fund"]["total_pnl"], -10.0, rel_tol=0.0, abs_tol=1e-12)
    assert isclose(groups["public_fund"]["unrealized_pnl_change"], -10.0, rel_tol=0.0, abs_tol=1e-12)
...
def test_period_calculation_groups_use_daily_risk_basis_for_daily_sources(
    ...
    for index, (instrument_id, instrument_name, instrument_type) in enumerate(
        [
            ...
            ("fund-us-daily-risk-test", "Daily Risk Test Fund", "public_fund"),
        ],
        start=2,
    ):
        transactions.append(
