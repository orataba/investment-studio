...
def _assert_0035_legacy_transaction_ledger_is_intact(database_url: str) -> None:
    ...
    try:
        with engine.connect() as connection:
            ...
            assert (
                calculation_state["refresh_completed_at"]
            ...
            connection.execute(
                text(
                    ...
                    INSERT INTO portfolio.portfolio_record (
                        ...
                        'Taxonomy Constraint Portfolio', 'USD', 'UTC',
                        'end_of_day', 0, 0
                    )
                    """
                )
...
def test_live_audit_enforces_research_backtest_metrics_contract(
    postgres_portfolio_env: dict[str, str],
) -> None:
    from portfolio_app.db import session as session_module
        ...
        *,
        ...
        return {
            ...
            "history_reliability": {
                ...
                "sample_label": "Canonical persisted Research history",
    ...
    valid_detail = {
        "backtest": {"metrics": short_metrics},
        "backtest_benchmark": {"metrics": eligible_metrics},
        "backtest_relative_metrics": unavailable_metrics,
    }

    portfolio_id = f"research-audit-portfolio-{uuid4().hex[:8]}"
    research_run_id = f"research-audit-run-{uuid4().hex[:8]}"
    session_factory = session_module.get_session_factory()
    with session_factory() as session:
        session.add(
            PortfolioRecordModel(
                portfolio_id=portfolio_id,
                portfolio_name="Research Metrics Audit Portfolio",
                base_currency="USD",
                valuation_timezone="UTC",
    ...
    def violation_count() -> int:
        with engine.connect() as connection:
            return int(
                connection.scalar(
                    text(
                        audit_module.PORTFOLIO_RESEARCH_BACKTEST_METRICS_CONTRACT_QUERY
                    )
                )
                or 0
            )
    ...
    def detail_with_invalid_metrics(
        ...
        if metric_path == "backtest.metrics":
            detail["backtest"]["metrics"] = invalid_metrics
        elif metric_path == "backtest_benchmark.metrics":
            detail["backtest_benchmark"]["metrics"] = invalid_metrics
        else:
            assert metric_path == "backtest_relative_metrics"
            detail["backtest_relative_metrics"] = invalid_metrics
        return detail

    legacy_metrics = deepcopy(eligible_metrics)
    legacy_metrics.pop("method_version")
    wrong_method_metrics = deepcopy(eligible_metrics)
    wrong_method_metrics["method_version"] = "research-backtest-metrics.v1"
    ...
    invalid_cases = [
        ("non_object", "backtest.metrics", []),
        ("unversioned", "backtest_benchmark.metrics", legacy_metrics),
        (
            "wrong_method_version",
            "backtest_relative_metrics",
            wrong_method_metrics,
        ),
        ("missing_history_field", "backtest.metrics", missing_history_field),
        ("wrong_minimum", "backtest_benchmark.metrics", wrong_minimum),
        (
        ...
        ("invalid_iso_boundary", "backtest_benchmark.metrics", invalid_iso_boundary),
        (
    ...
    try:
        ...
        with session_factory() as session:
            ...
            row.detail_json = {
                ...
                "backtest_benchmark": {"metrics": None},
