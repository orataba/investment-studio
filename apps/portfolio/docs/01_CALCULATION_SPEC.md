...
- 每个父节点用本节点的配置依据解释直接成员目标。Weight 模式施加硬等式 `W_child = target_value_child × W_parent`；Risk 模式以 `Q_child = target_value_child × Q_parent` 为目标。固定权重 sleeve 是按配置比例组成、整体可缩放的篮子；signed Euler 模式下与先合成 `r_B = Σ p_j r_j`、再将 B 和其他风险单元共同求解等价，完整保留跨 sleeve 协方差。一个求解同时处理不同层级的资金／风险关系，不先独立求子组合风险预算再拼接，也不对每一层重新 shrink covariance。
...
- Weight 比例、冻结、资金和显式上下限均为硬约束，彼此冲突时明确不可行，不能为改善风险预算偏差而改变固定篮子比例。硬约束满足、风险父节点贡献为正且数值求解收敛时，存在风险预算偏差的结果标为 `constrained_solution`，可继续研究模拟并披露偏差，不宣称全局最优；未收敛且未达标、零／负父节点风险贡献等结果仍不可执行。`satisfied` 只用于目标实际达成的解。
...
求解展示保存 `solution_tree`，层级来自本次冻结分类，每个叶只计入一次，父行仅汇总直接子行。证券账面金额为市值，现金含待交收货币项目，衍生品沿用 signed carrying capital；当前权重为当前账面金额 / 求解当日 statement NAV，目标权重为求解目标金额 / 同一 NAV，调仓金额仅显示带符号的目标金额减当前账面金额。不交易成员金额不变，期权负债已在 statement 的负持仓行中计入，不能再扣汇总披露额。目标风险贡献从保存的组合根预算沿连续 Risk 路径相乘，分叉的 Weight 层不能推出叶级风险目标；真正单成员的 Weight 恒等路径可保留上层预算。分类求解贡献汇总保存的成员贡献，已建模且求解权重明确为零的成员及结构空分类贡献为零；缺少非零成员归因仍为未知。旧运行只读保存值与保存的配置，不能调用当前分类或行情重建历史；旧目标 schema 不按新规则重解释，只能从保存的根预算及完整路径证明目标。未保存 NAV 时，仅当记录明确为全组合且全部账面金额与保存权重逐项对账成立，才恢复 NAV；否则当前权重未知，目标权重保留当时研究范围。子范围存档的求解贡献仍为该范围归因，不伪称全组合贡献，也不与全组合目标直接算偏差。缺币种时原始金额照录并说明未换算。Excel 导出完整树及其币种、日期、范围、状态和数值，不受页面折叠影响。
...
`PUT /api/portfolios/{portfolio_id}/research/settings` 的 core run-setup 字段使用全量 PUT 语义，调用方应提交完整 core settings；`frozen_taxonomy_node_ids` 和 `top_sleeve_weight_bounds` 省略时保留、空列表时清空。`backtest_*` control 省略时保留当前值；nullable benchmark 显式 `null` 时清空。API 不得把省略的 backtest control 静默重置为 schema default。

共同区间的收益由两条收益指数在相同收盘边界重新计算；可选基准须覆盖整个共同区间，不能缩短真实／回测窗口。年化收益遵循 calendar-anniversary Actual/Actual，满一周年才展示，末端零值保留 -100% 总损失；风险年化使用共同收益观测数 / 实际锚点到期末天数 × 365.25，Sharpe 的无风险收益与 Sortino 的最低可接受收益为 0。最大回撤时点、修复天数和缺口处理沿用 Performance 约定。月度收益矩阵按年度分组，每年包含真实、回测、差值及可选基准，横向为 1–12 月和年度累计（当年为截至所选日期的累计），各序列保留自己的完整可靠历史，不因其他序列较晚开始而截短。月度与年度收益分别从前一期末可靠收盘边界计算指数比值，年度不得把月收益算术相加或当作年化收益；没有前一期末有效边界时只展示本期内已有观察的收益并标示实际区间，稀疏跨月／跨年变动不得归入单一期间。缺失月份留空，差值仅在两条路径的起止边界相同时发布。周末期末可使用前一工作日收盘；无法确认的其他期末缺口保留区间提示。共同区间指标仍使用其明确标示的更窄窗口。
...
- 支持 `1w`、`1m` 与 `3m` rebalance。候选调仓日可以使用此前已经存在的完整风险历史，不需从分类或目标创建日重新积累一个 lookback。窗口保留起点之前最近的收盘锚点，不把末端缺失净值无限前填成零收益。组合成员共同历史不足所需 risk window 时跳过该次决策并给出 warning，初始持仓仍按可用的真实行情继续估值，不生成晚于 as-of 的日期，不以更短风险窗口替代；benchmark 历史只影响 benchmark 曲线和相对指标，不得推迟或阻断组合自身模拟起点；
...
- 同次核算内共用证券明细及按报价基准建立的日期索引，清洁序列校验后用日期查找逐日值；若准备窗口存在坏数据，仍按每个实际历史日执行原有严格判断，不能把未来问题提前传播给此前有效日期，也不能绕过当前日缺价。初始买入明细只批量加载原绑定规则确实会读取且尚未加载的证券，不扩大到无关或不合资格的交易资产；已有明细和未找到的结果在当次核算内共用。共享明细读取避免重复行解包、NAV 校验和序列化，保持原始金融排序及来源字段。FX 仅按实际直接、反向或 USD 中转路径加载，但候选资格仍执行完整历史校验；基准币种现金不触发无关 FX 读取。该复用仅存在于一次计算，不改变报价选择、时钟、现金流、收益或风险公式。
