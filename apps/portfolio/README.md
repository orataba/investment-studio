...
- Holdings 简版、完整风险视图和尾部风险共享完整持仓分析结果，summary 共用的风险频率同样按快照来源版本缓存；日期、风险政策、分析分类版本参与缓存键。同一版本的并发请求共用在途计算，不阻塞其他组合；计算中来源发生变化的结果不缓存，等待者重新核对来源，金融响应发布前再次核对来源。实时任务和衍生品观察在缓存外读取，页面裁剪仅修改独立响应；尾部风险复用持仓收益区间，仅额外读取证券频率／日历和 FX 历史。展示图表裁剪不裁剪收益率、回撤或波动率计算历史；
