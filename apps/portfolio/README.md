...
- 交易变更有可靠前缀时，仅从最早受影响日的前一核算边界重算尾段；前缀累计收益和回撤高点只读取必要字段。一次核算共用证券明细与报价日期索引，清洁序列只校验一次；只读取实际币种路径需要的 FX，保留完整历史资格、直接／反向／USD 中转顺序及缺口停算规则。共享明细读取减少重复行解包、规范化和 NAV 校验，金融排序与来源信息不变。`portfolio_snapshot_initial_market_inputs`、`portfolio_snapshot_kernel`、`portfolio_snapshot_publication` 分别记录初始行情读取、核算和发布耗时与失败类型，不记录交易金额；
...
- Holdings 简版、完整风险视图和尾部风险共享完整持仓分析结果，summary 共用的风险频率同样按快照来源版本缓存；日期、风险政策、分析分类版本参与缓存键。同一版本的并发请求共用在途计算，不阻塞其他组合；计算中来源发生变化的结果不缓存，等待者重新核对来源，金融响应发布前再次核对来源。实时任务和衍生品生命周期在缓存外读取，挂钩证券仅复用同一日期和来源版本的执行报价；精简持仓先在隔离容器内裁剪，再复制保留的响应字段。尾部风险复用持仓收益区间，仅额外读取证券频率／日历及实际使用的直接、反向或 USD 中转 FX 历史，同一次请求的币种路径只解析一次；FX 全历史有效性校验保留。展示图表裁剪不裁剪收益率、回撤或波动率计算历史；
...
- 账户、持仓及批次列表共用已校验的单日官方估值报价，按资产 ID、日期、数据库及行情／配置来源水位立即失效；首个新来源版本仍校验完整历史。初始买入价格、交割排除、批次数量、成本和现金事实均在请求内处理，报价复用不等待每日快照。账户 FX 按请求内实际日期与币种对复用解析结果。报价与工作区共用最多 64 项、近似 JSON 序列化总量 32 MiB 的 LRU；超总预算的结果仅合并在途计算，不驻留缓存；
