...
- Short option 行显示 Call/Put 合约数、到期日、行权价、乘数、到期对应标的数量和剩余权利金负债，不分配或校验标的覆盖。期权关闭与股票买卖、FCN 关闭与接收资产均为独立交易；需要说明时使用 note。
