...
- `Holdings` 的物化行包含 instrument market profile、analytics scope identity、四区 operational fields 与 `risk_basis` 对齐元数据；动态、物化和 instrument-detail 读路径必须返回同一合同。`risk_basis` 只描述 `risk_eligible=true` 市场持仓的 daily / weekly / monthly return alignment，不是风险指标；频率优先使用 Registry expected frequency，daily 数据按各自 market calendar 校验预期 session。若任一 eligible member 缺少可解析来源频率或存在预期观测缺口，即使所有成员共同缺同一天，Risk 页面和 Forward RC 也必须标记 basis incomplete。Holdings 按 Market-valued、Structured / Long Derivatives、Written Option Obligations、Cash & Settlement 四区展示；总 NAV 包含 market value、event carrying assets、writer liabilities、settled cash 和 pending settlement。
- written-option coverage 在同一 account/underlying pool 中按最早 expiry 优先且每单位标的只分配一次；页面始终显示 uncovered count，汇总 expiry buckets、assignment notional、settlement receivable/payable/net，并对 uncovered、到期、逾期和 FX unavailable 返回 line-addressable alerts。Regions 导出固定 24 列 canonical schema，并以 `N/A` 表示不适用；Advanced Table 只导出当前可见列。
...
- `Holdings` 中普通市场资产的 `Market Value Base` 是 base-currency fair value，现金行使用 settled cash 的 base value。事件型 FCN/长期权使用 `valuation_basis=carried_cost`，期权卖方义务使用 `valuation_basis=premium_liability` 和负的 NAV amount；二者的 `fair_value`、quote identity、Day Change / Day Return 均为空，也不进入 Priced Lines 或风险样本。Forward Risk 只对 effective-dated taxonomy policy 标记 `risk_eligible=true` 的 modeled sleeve 建 covariance；policy-excluded event/liability 不阻断该 sleeve，但必须披露 `excluded_carrying_value`、`excluded_liability`、coverage ratio 和 excluded rows。没有 eligible risky holding，或 eligible member 的 return/FX/period identity 不完整时才 unavailable，不能把未知收益补成零。普通市场资产的 Day change 优先使用拆分和分配处理后的 total-return basis，缺失时才回退到 valuation basis，并把该日收益应用到 as-of date 当前持仓规模；非 base cash 使用 FX 变动。Instrument `1W / 1M / 3M / 6M / MTD / YTD / 1Y Return`、volatility 和 drawdown 只使用 Registry 确认的 total-return series，按策略顺序锁定单一 basis；不能用普通价格序列或 sampled price chart 替代。Portfolio 与 Watchlist 各自实现，但同一 instrument、as-of 和窗口必须得到相同 total return。Holdings group / subtotal / total 的 return 使用当前 base-value 权重，volatility / drawdown 基于当前权重合成的组 return series 计算，不使用成员风险指标的加权平均，也不表示历史实际组合 TWR。Holdings 主表的 locked `instrument` 首列在横向滚动时保持冻结；只有当前视图需要非默认 chart 或组合波动率 / 回撤时才请求 full return-series payload。
...
- `Risk` Current Drift 在 instrument-scope taxonomy 下把非现金 Holdings rows 与全部 Accounts 的 base-currency cash + pending settlement 合成当前 NAV；证券账户 positions 只通过 Holdings 计入一次。现金只参与 capital / weight drift；一个直接 cash bucket 会与当前汇总现金比较。`Risk Target Gap` 只比较 `risk_budget_eligible=true` 的非现金成员，现金不建 risk target share、不进入风险预算 100% 分母。Forward covariance 只在 `risk_eligible=true` leaf instruments 上运行一次，权重按 eligible gross exposure 归一，taxonomy risk share 按同一 modeled-sleeve variance 分母加总；excluded exposure 和 cash 通过 coverage disclosure 保留，不作为 0-return member。
