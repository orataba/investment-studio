...
- 复用前缀还必须确认其计算版本、市场来源、证券配置来源及分类配置版本与本次一致；证据缺失或来源不同则完整重建。交易产生的新请求 ID 不参与此前缀比较，来源未变的普通交易仍可增量；新证券扩大来源范围、行情或配置更新却无法证明旧前缀仍有效时，保守完整重建。计算中迟到的来源变化沿用已有丢弃与重试流程。`portfolio_snapshot_prefix_rejected` 记录拒用原因和来源字段类别，不记录交易、金额或源数据内容；
...
- 分类、归属和目标统一使用当前配置，无生效日期或历史区间；各页面选择所需分类，没有全局默认规划分类或规划资格开关，`taxonomy_configuration_version` 标识分类配置来源。风险资格由真实资产、估值和数据覆盖派生，不受分类归属和目标控制；修改后完整重建受影响的历史派生读模型。Research 每次冻结完整当前配置，当前求解及全部历史调仓共用该快照；Pinned 仅限制金融数据截止日。已保存研究结果和报告保留原快照，原始金融事实日期不变；
...
- 条件不足的收益、风险和研究结果明确 unavailable，不用旧算法、等权或不完整样本兜底。Research 的当前性校验同时覆盖当前分类目标、历史交易行版本、账户及合约、本位币和风险设置、研究标的及 FX 的共享来源版本；计算过程中这些输入变更会终止本次发布并保留上一份有效结果。来源水位不提供按日期的局部修订信息，因此固定分析日的研究在相关来源更新后也需重算确认。
...
Risk 的“集中度”展示单证券 gross 市值、单 FCN 剩余本金及自定义分类敞口。FCN 本金按挂钩标的等分或按自定义比例分配一次，Option 不进入分子；分母是现有账面口径的组合 NAV。集中度有独立截至日期；风险页其他指标仍使用当前持仓日期。Overview 仅显示超限和无法判断的摘要，并链接到同日集中度。DSH 使用同一投影；缺分类、缺估值或缺收益数据不能解释为零风险。

Taxonomies 在同一展开树表并列维护 SAA/TAA，父行 `allocation_basis` 控制直接成员唯一 `target_value` 的解释。TAA 整层全空继承 SAA，部分填写必须补全；移走成员只删除其旧层级目标，保留其他成员原值并要求重新补足，空层级不留下无法编辑的目标；移动分类保留内部目标。根 Cash 是独立 NAV 预留，证券目标合计 100%，FCN/Option 不设可编辑目标。没有默认规划分类、独立维度启用开关或 Research 维度覆盖。各层依据、目标与逐项集中度上限一次原子保存，保存时核对编辑开始的配置版本以防覆盖其他编辑者；仅限额变更不增加分类版本或使研究过期。

集中度仅有逐对象上限和 taxonomy 级提醒开关。单证券／单 FCN 上限全组合唯一，节点上限属于各自分类。空白不限制，0 禁止正敞口，严格超过才超限；没有通用值、继承/覆盖或关注线。限额按展示估值日生效，历史 revision 和 FCN 分配仍独立持有日期审计。组合复制保留全部历史及未来限额，并将分类、节点引用映射到副本；手动加入的候选标的也保留。
Research 在选中研究范围中使用一份全局 leaf covariance，同时满足各父节点的资金或 Euler 风险贡献比例，最终权重与层级诊断使用同一风险口径；集中度上限不自动成为求解硬约束。运行保存 `solver_version` 和风险归因范围，算法版本进入输入指纹及缓存身份；旧局部求解结果保持存档但明确过期。方法与约束详见计算规格第 10.7 节。
