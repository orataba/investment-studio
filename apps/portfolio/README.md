...
Holdings 的“敞口与集中度”展示单证券 gross 市值、单 FCN 剩余本金及自定义分类敞口。FCN 本金按挂钩标的等分或按自定义比例分配一次，Option 不进入分子；分母是现有账面口径的组合 NAV。Overview 仅显示超限和无法判断的摘要。DSH 使用同一投影；缺分类、缺估值或缺收益数据不能解释为零风险。
