...
from platform_app.services import instrument_reference
...
def test_tushare_index_reference_uses_profile_and_latest_constituents(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    monkeypatch.setattr(
    ...
    def fake_rows(**kwargs: object) -> list[dict[str, object]]:
        ...
        return [
            {"trade_date": "20260731", "con_code": "000001.SZ", "weight": "1"},
            {"trade_date": "20260821", "con_code": "600519.SH", "weight": "5"},
            {"trade_date": "20260821", "con_code": "300750.SZ", "weight": "3"},
        ]
    ...
    assert result is not None
    assert calls == ["index_basic", "index_weight"]
    assert result["sections"]["index_info"]["name"] == "沪深300"
    assert [
        row["con_code"] for row in result["sections"]["constituents"]
    ] == ["600519.SH", "300750.SZ"]
    assert result["section_errors"] == {}
