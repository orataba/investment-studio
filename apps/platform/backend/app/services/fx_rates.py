...
from app.services.instrument_store import get_instrument, upsert_market_data
...
def _latest_spot_point(asset_id: str) -> dict[str, object] | None:
...
def _direct_rate_record(base_currency: str, quote_currency: str) -> dict[str, object] | None:
    asset_id = MAINTAINED_FX_INSTRUMENTS.get((base_currency, quote_currency))
    if asset_id is None:
        return None

    point = _latest_spot_point(asset_id)
    if point is None:
        return None
...
def _inverse_rate_record(base_currency: str, quote_currency: str) -> dict[str, object] | None:
    direct_record = _direct_rate_record(quote_currency, base_currency)
...
def _cross_rate_record(base_currency: str, quote_currency: str) -> dict[str, object] | None:
    if base_currency == PIVOT_CURRENCY or quote_currency == PIVOT_CURRENCY:
        return None

    usd_to_base = _direct_rate_record(PIVOT_CURRENCY, base_currency)
    usd_to_quote = _direct_rate_record(PIVOT_CURRENCY, quote_currency)
...
def get_fx_rate(base_currency: str, quote_currency: str) -> dict[str, object] | None:
    ...
    direct_record = _direct_rate_record(normalized_base, normalized_quote)
    if direct_record is not None:
        return direct_record

    inverse_record = _inverse_rate_record(normalized_base, normalized_quote)
    if inverse_record is not None:
        return inverse_record

    return _cross_rate_record(normalized_base, normalized_quote)
    ...
    for base_currency in SUPPORTED_FX_CURRENCIES:
        for quote_currency in SUPPORTED_FX_CURRENCIES:
            if base_currency == quote_currency:
                continue
            record = get_fx_rate(base_currency, quote_currency)
...
def upsert_fx_rate(
    *,
    ...
    refreshed = _direct_rate_record(normalized_base, normalized_quote)
