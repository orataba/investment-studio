...
class Instrument(InstrumentRegistryBase):
    ...
    market_data_updated_at: Mapped[str | None] = mapped_column(String)
    calculation_inputs_updated_at: Mapped[str | None] = mapped_column(String)
...
class InstrumentReferenceSnapshot(InstrumentRegistryBase):
    """Latest collected provider reference data, independent of pricing facts."""

    __tablename__ = "instrument_reference_snapshot"
...
class InstrumentBrokerIdentifier(InstrumentRegistryBase):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "broker = lower(trim(broker)) AND identifier_type = lower(trim(identifier_type)) "
            "AND identifier_value = lower(trim(identifier_value))",
            name="broker_identifier_normalized",
        ),
        UniqueConstraint(
            "broker",
            "identifier_type",
