...
class Settings(BaseSettings):
    app_name: str = "Investment Studio Data"
    ...
    database_schema: str | None = "instrument_data"
    operations_database_schema: str | None = "data_ingestion"
    sql_echo: bool = False
    ...
    model_config = SettingsConfigDict(
        env_prefix="INVESTMENT_STUDIO_DATA_",
        extra="ignore",
    )
    ...
    def _coerce_database_schema(cls, value: object) -> object:
        ...
        if isinstance(value, str):
            ...
            if not normalized:
                return None
            if normalized != "instrument_data":
                raise ValueError(
                    "database_schema must be the canonical 'instrument_data' schema."
                )
            return normalized
        return value
    ...
    def _coerce_operations_database_schema(cls, value: object) -> object:
        ...
        if isinstance(value, str):
            ...
            if not normalized:
                return None
            if normalized != "data_ingestion":
                raise ValueError(
                    "operations_database_schema must be the canonical 'data_ingestion' schema."
                )
            return normalized
        return value
    ...
    def resolved_fmp_api_key(self) -> str:
        ...
        if self.fmp_api_key_file is None:
            raise ValueError(
                "Configure INVESTMENT_STUDIO_DATA_FMP_API_KEY_FILE or "
                "INVESTMENT_STUDIO_DATA_FMP_API_KEY."
            )
