...
from studio_data.services.email_ingestion import (
    EmailIngestionBusyError,
    EmailIngestionError,
    ingest_email_nav,
)
from studio_data.services.email_ingestion.repository import EmailIngestionRepository
...
LOGGER = logging.getLogger("investment_studio.market_data_ops")
...
def refresh_market_data(
    *,
    ...
    if requested_source == "fmp":
        ...
        from studio_data.services.securities import refresh_security_eod
    ...
    if source_mode == "api":
        ...
        return update_refresh_status(
            ...
            status="blocked",
            message=(
                f"API refresh profile {profile or 'unconfigured profile'} is not supported. Configure a supported source through backend maintenance."
            ),
            updated_by=updated_by,
            mode="api",
        )

    location = str(source_settings.get("source_location") or "Backend CLI").strip()
    return update_refresh_status(
        instrument_id=instrument_id,
        status="awaiting_manual_import",
        message=f"Manual source active. Import canonical NAV from {location} through backend maintenance.",
        updated_by=updated_by,
        mode="manual",
    )
    ...
    *,
    ...
    try:
        ...
        if instrument_type == "index" and suffix in TUSHARE_INDEX_SUFFIXES:
            ...
            prepared_rows = _tushare_price_rows(
                rows,
