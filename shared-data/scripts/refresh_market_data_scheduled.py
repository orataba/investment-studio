...
STATE_ROOT = Path(os.environ.get("INVESTMENT_STUDIO_STATE_DIR") or (
    Path(os.environ.get("XDG_STATE_HOME", str(Path.home() / ".local/state")))
    / "investment-studio"
))
sys.path.insert(0, str(BACKEND_ROOT))

from studio_data.services.downstream_notifications import (  # noqa: E402
...
LOGGER = logging.getLogger("investment_studio.market_data_refresh")
...
def _database_url() -> str:
    return str(os.getenv("INVESTMENT_STUDIO_DATA_DATABASE_URL") or "").strip()
    ...
    *,
    ...
    if not database_url.startswith(("postgresql://", "postgresql+psycopg://")):
        raise DatabaseUnavailableError(
            "INVESTMENT_STUDIO_DATA_DATABASE_URL must identify PostgreSQL."
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "then runs tushare, email, used-security FMP EOD, fund NAV projection "
    ...
    parser.add_argument(
        ...
        type=float,
        default=os.getenv(
            "INVESTMENT_STUDIO_LOCAL_REFRESH_DATABASE_WAIT_SECONDS",
            "300",
        ),
    ...
    parser.add_argument(
        ...
        type=float,
        default=os.getenv(
            "INVESTMENT_STUDIO_LOCAL_REFRESH_DATABASE_RETRY_INTERVAL_SECONDS",
            "5",
        ),
...
def _channels(selected_channel: str) -> list[str]:
    if selected_channel == "all":
        return ["fmp_catalog", "tushare", "email", "fmp", "fund_nav_projection", "reference"]
...
def _run_refresh(
    ...
    *,
    ...
    for channel in channels:
        ...
        if channel == "reference":
            response = refresh_reference_data_batch(instrument_ids=requested_instrument_ids or None)
            results = response["results"]
            # Reference snapshots do not change pricing or trigger recalculation.
            all_results.extend(item for item in results if item["status"] == "failed")
            channel_summaries.append({"channel": channel, "result_count": len(results),
