...
def _alembic_config(
    ...
    *,
    ...
    monkeypatch.setenv("INVESTMENT_STUDIO_INSTRUMENT_DATA_SCHEMA", "instrument_data" if schema else "")
...
def postgres_registry_contract_schema(
    ...
    database_name = f"registry_contract_{uuid4().hex}"
    database_url = sa.engine.make_url(base_database_url).set(database=database_name).render_as_string(hide_password=False)
    admin_engine = sa.create_engine(base_database_url, isolation_level="AUTOCOMMIT")
    try:
    ...
    engine = sa.create_engine(database_url)
    schema = "instrument_registry"  # Historical revisions precede the schema rename.
...
def test_option_contract_identity_migration_on_postgresql(
    ...
    command.upgrade(config, "20260902_0029")
    with engine.connect() as connection:
        _set_search_path(connection, schema)
        assert connection.scalar(sa.text("SELECT version_num FROM alembic_version")) == (
            "20260902_0029"
