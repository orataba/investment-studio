...
from studio_data.core.settings import Settings
from studio_data.db.session import _search_path_fragments
...
def test_settings_define_a_private_operations_schema_and_migration_target() -> None:
    ...
    assert settings.database_schema == "instrument_data"
    assert settings.operations_database_schema == "data_ingestion"
...
def test_platform_search_path_precedes_the_shared_registry() -> None:
    assert _search_path_fragments(
        "data_ingestion",
        "instrument_data",
    ) == ["data_ingestion", "instrument_data", "public"]
    assert _search_path_fragments(
        "instrument_data",
        "instrument_data",
    ) == ["instrument_data", "public"]
...
def test_email_ingestion_folders_accept_csv_from_environment(
    ...
    monkeypatch.setenv(
        "INVESTMENT_STUDIO_DATA_EMAIL_IMAP_FOLDERS",
        "INBOX,云谷3号,策略精选1号",
    )
