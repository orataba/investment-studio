...
from studio_data.commands import status
...
def test_email_nav_inventory_enforces_limit(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    captured: list[int] = []
    ...
    monkeypatch.setattr(status, "get_session_factory", lambda: object())
    monkeypatch.setattr(status, "build_email_nav_inventory", fake_inventory)

    assert status.get_email_nav_inventory(limit=500) == {
        "limit": 500,
        "counts": {},
    }
    assert captured == [500]
    with pytest.raises(ValueError):
        status.get_email_nav_inventory(limit=501)
