...
const systemPatterns: LanguagePattern[] = [
  ...
  { match: /^(Current risk|Forward RC) cannot treat unmodeled market exposure ([^·]+) as zero risk\.$/, replace: (model, name) => `${model === 'Current risk' ? '当前风险' : '前瞻风险贡献'}不能将未能建模的市场敞口 ${name} 视为零风险。` },
  ...
  return supportedLanguages.reduce<LanguagePatternMessages>((merged, language) => {
    merged[language.value] = [
      ...(language.value === 'zh-Hans' ? systemPatterns : [
        ...
        { match: /^Sort (.+?)(:?) (ascending|descending|no sorting)$/, replace: (label: string, separator: string, direction: string) => `Sort ${systemSourceAliases[label] || systemEnglishLabels[label] || label}${separator} ${direction}` },
        { match: /^Resize (.+) column$/, replace: (label: string) => `Resize ${systemSourceAliases[label] || systemEnglishLabels[label] || label} column` },
