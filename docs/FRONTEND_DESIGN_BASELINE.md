...
- Research 求解结果使用单一多级树表，分类与标的不拆成重复的两张表；折叠全部仍保留一级分类。列为分类／标的、目标风险贡献、求解风险贡献、账面金额、当前权重、目标权重、带符号调仓金额；当前与目标权重均以保存的组合 NAV 为分母，不混入经济敞口。目标风险贡献须有从组合根预算出发的完整依据，分类求解贡献汇总成员在同一保存范围中的贡献；旧档无法证明的值留空，子范围口径用统一提示说明。金额、数字和编辑控件遵循同一整行层级字重。Excel 导出包含完整树、数值、报告币种、运行身份及口径，不受当前展开状态影响。
- Research 只保留一组真实组合／回测／可选基准曲线、共同区间指标和月度收益矩阵（年份分组，真实／回测／差值逐行，1–12月及年度累计逐列）；旧运行仍保留审计记录，不另堆一组“存档历史回测”图表。成立日、实际有效范围和共同比较窗口明确区分，模型说明集中到圆圈提示，实际缺口、过期和不可执行状态仍可见。
...
- 所有实际横向溢出的数据表统一使用鼠标左键拖动普通单元格横移，Shift + 拖动可选取文字；没有溢出时保留原生文字选择。表头排序、名称链接、按钮、输入/选择器、列宽调整和已有原生拖放不被横移拦截。真正拖动后不得误触行跳转。触屏与触摸板保持浏览器原生滚动，溢出容器支持 Tab 聚焦与方向键滚动。React 页面复用 `HorizontalTableScroll` 或同一 `useHorizontalTablePan`；Regime 原生页面保持同样的合同。
