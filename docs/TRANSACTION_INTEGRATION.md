...
The guided Excel workbook contains five sheets: `Instructions`, blank `Transactions`, `Field Guide`, `Examples`, and `Lists`. Dependent dropdowns first select the asset and then show only that asset's actions. Structural validation also covers fee categories, USD/HKD/CNY/EUR/GBP/CHF, option types, dates, times, and non-negative numbers. It intentionally contains no portfolio, account, instrument, or existing-contract data. All IDs in `Examples` are placeholders and must be replaced with IDs that already exist in the target portfolio.

Supported transaction currencies are exactly `USD`, `HKD`, `CNY`, `EUR`, `GBP`, and `CHF`. A holding account, its security or derivative contract, the settlement cash account, and the transaction must be currency-compatible. Cash internal transfers are same-currency only; position transfers require the instrument and both security accounts to use the same currency. For FX conversion, `currency` is the source cash-account currency, the target currency comes from `counterparty_account_id`, and `counter_amount = gross_amount * fx_rate`.

The examples cover every non-transfer action for Cash, Security, FCN, and Option, plus one copyable `transfer_out` example for each transfer domain. A transfer command is entered once; do not copy both directions. Physical option exercise or assignment remains an Option cash-settlement action plus an independent underlying Security action. FCN asset delivery remains an FCN close action plus an independent Security `buy`; neither workflow creates a pairing or relation ID. `opening_balance` represents an existing long option position and must use the portfolio inception date for both trade and settlement. An existing written position must be backfilled with `sell_to_open` using its actual open date, quantity, and premium; it must not be represented as a positive-quantity opening balance.
...
Transfer commands may carry `source_system` and `external_reference`. The command identity is persisted on the generated `transfer_out` leg, retained by collapsed export, and rejects the same source fact even when a later import uses another batch Idempotency-Key. Cash Fee/Tax rows do not carry `entitlement_date`; asset-linked fee/tax rows do and require an entitled long position or written-option obligation. A standalone Option Fee/Tax is a contract-level cash expense and does not alter a long lot or writer obligation. Return of Capital uses `trade_date` as its entitlement/record date and `settlement_date` as its cash receipt date.
