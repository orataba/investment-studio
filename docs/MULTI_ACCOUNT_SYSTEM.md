...
| 范围 | 读取 | 修改与归属 |
| --- | --- | --- |
| Home | 本人账号；团队成员目录 | 管理员邀请、停用、重置；拥有者交接；本人登录名、显示名和密码 |
...
- 浏览器写入请求检查 Origin；后台 Bearer 请求独立验证身份和范围。已登录修改密码无额外二次验证，但在用户锁内重新校验当前会话；密码更改与并发重置只允许一个操作成功。修改登录名不改永久用户 ID、历史署名或组合权限。
- 鉴权结果不跨请求缓存。共享客户端复用到 Home 的连接，用户、团队和成员身份通过联表读取；停用、撤销或角色变化在后续请求立即生效。
...
1. 备份项目数据库及外部配置，停止旧服务的写入。备份/恢复脚本已包含 `identity` schema。当前迁移移除二步验证，回退须使用迁移前备份。
...
6. 按已确认的角色建立团队成员与只读成员。受邀人通过一次性链接自行设置登录名和至少 8 位密码，激活页展示对应人员；密码重置页展示已有登录名。新成员默认不预授现有组合权限，之后由组合管理者手工授予。邀请链接仅创建并呈现给管理员，没有自动发邮件或发消息。
...
- Watchlist 的新身份迁移不提供静默降级。确需回退时使用停写后的完整备份，并一并恢复对应会话状态。
