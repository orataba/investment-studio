...
`request_payload_json` records the immutable current-target snapshot, its fingerprint, `planning_state_fingerprint_version` and `solver_version`. The current global method is `global_leaf_scalar_targets_v4`; its identity also participates in the derived-analysis cache fingerprint. Current target snapshots use schema 3 and planning fingerprints use version 7. A missing or earlier method/input-identity version makes the saved result stale without rewriting its archived payload or artifacts.

New `detail_json.solution_tree` freezes the result hierarchy, recorded amounts and exposures, and the solve-date statement NAV used for capital weights. Archived results use only saved fields and their recorded weight basis; missing NAV or exposures remain unknown, and older target schemas are not reinterpreted with current rules. Simulation starts at portfolio inception; pre-inception prices are risk-estimation warm-up only.
