...
本地 Regime 使用 `studio_market_regime_reader` 读取共享数值。该账号只获 `market_data`
schema 的 `USAGE` 和表的 `SELECT`，不获私有账本权限。市场库迁移会在该账号已存在时
重设读取权限及新表的默认读取权限；恢复流程省略 ACL 后也须执行现有统一迁移步骤。
...
大规模首次入站包会额外占用一份空间。本次 bootstrap 在完整传输、校验并逐批确认导入后，持发行锁将原 ZIP 在同一文件系统内移入 `numeric/outbox`，按原 manifest 的批次集合、字节数与 SHA256 原子登记正式索引及校验文件，不重新导出另一份大包。这样只保留规范数据和正式发行包两份；本地已具备全部批次时直接跳过该包下载。
