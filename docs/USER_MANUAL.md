...
点击“编辑目标”后，在“目标层级”选择需要配置的范围，分别启用战略／战术的权重和风险贡献，再填写该层直接成员的目标。切换查看节点不会覆盖正在编辑的目标；保存整体提交，取消恢复已保存配置。

删除分类节点须输入节点名称确认。删除范围包括该节点及全部子节点、相关归属、这些节点的目标行及其子层目标配置；保留其他节点的原目标，不自动重新分配比例。相关证券回到未分类，需重新检查归属和目标完整性。实际交易、持仓和已保存研究快照保留，当前分析按删除后的配置重算。

若 Research 报 scope 无成员、目标缺失或 top sleeve bounds 无法满足，通常要回到 Taxonomies 检查 assignment、TargetSet 和树结构。

分类、归属、目标和分析范围都只有当前配置，无须填写生效日期。一次保存会整体提交修改并自动安排完整历史重算，使 Holdings、Risk、Performance 和 Research 一致使用当前分类。历史分类归因会随新配置重述；原始交易、现金流、行情和净值事实不改写。保存失败不会部分生效。既有研究结果和已保存报告保持原快照，Research 需重新运行后才生成使用新目标的结果。
...
Research 可以选择任意活动分类；无目标分类可以浏览和返回分类设置，正式运行要求当前已保存的目标完整。每次运行将当前分类、标的归属、目标和求解资格保存为一份快照，当前求解与整段历史回测均使用这份快照。目标不再按历史生效日期切换。

页面中的日期只表示行情和持仓数据截止日，可以使用最新可用数据或指定历史截止日；目标始终使用本次运行时的当前配置。历史曲线回答“当前目标在这段历史中会怎样”，不表示当时已经采用该目标。旧结果保持原样并展示存档方法；重新运行才会采用新目标。交易、目标、分析设置或所需行情更新后，旧结果会提示过期；计算过程中输入改变会终止本次发布，保留上一份有效结果。
