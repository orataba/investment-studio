...
- 集中度明细位于 Risk；Overview 仅提示超限和已设上限但数据不足的项目，可直接打开同日集中度。集中度可独立选择截至日期，风险页其他指标仍使用当前持仓日期。Risk 的 Top-3/HHI 是建模证券内部的持仓分布摘要，不套用 NAV 集中度上限。
...
Risk 的集中度明细显示敞口、上限、余量及来源，可切换单证券、单 FCN 和各分类。FCN 本金分配在单 FCN 视图内编辑。目标与限额可在树表统一保存；限额从页面所展示估值日起生效，保存旁明确显示日期。历史查看使用该日已生效的限额，原修订保留审计；修改限额或分类提醒开关不会使 Research 过期。
...
Taxonomies 管理分类树、证券归属、目标和逐项集中度上限。分类可按国家、行业、策略等需要建立；没有全局“默认规划分类”或“允许规划”开关。Holdings 和 Performance 的 Group By 直接列出当前活动分类，保存视图会记住具体分类。Overview、Risk 和 Research 分别使用本页选择；分类删除或停用后，各页不会改用另一套分类。仅为 FCN 关联而出现的标的可以分类，但不会自动成为可投资研究候选；通过“添加标的”明确加入时，会纳入组合观察范围，允许配置其直投目标。
...
金额列与 Holdings 使用同一组合报告币种；证券显示市值，衍生品显示账面金额，币种标在列头，不重复附在标的名称后。缺少换算数据时显示不可用，不把原币金额直接标成报告币种。根分类、一级分类、子分类和标的通过名称字重与缩进区分，金额不随层级加粗。进入编辑保留表格列宽与行高。
...
分类、归属、目标都只有当前配置，无须填写生效日期。修改这些配置会使相关动态历史展示重算、已有研究结果提示过期；原始交易、现金流、行情和净值事实不改写，已保存研究和报告保持原快照。仅改限额不触发该重算或研究失效。限额自身保留日期与修订，生效日说明位于集中度列头的圆圈提示中。

删除分类节点须输入名称确认，范围包括全部子节点、归属及相关目标。其他节点不自动重新分配比例，受影响证券回到未分类。删除后同名重建获得新身份，不继承旧对象的限额；新节点默认没有上限，旧修订和研究快照保留原引用。

市场风险覆盖由真实持仓、估值和收益／汇率数据决定，不设置人工风险资格、业绩范围或估值标签。未分类或目标为零的实际证券持仓仍参与可支持的风险计算；缺数据或模型不支持时，显示未覆盖敞口和原因。组合优化所需分类与目标不完整时明确不可求解，不以等权或零风险替代。

### 6.9 组合优化（Portfolio Optimization）

组合优化用于 target solve 和假设回测。它不改变真实持仓，结果用于研究和调仓建议。

组合优化可以选择任意活动分类；无目标分类可以浏览和返回分类设置，正式运行要求当前已保存的目标完整。每次运行将当前分类、标的归属、目标和研究成员保存为一份快照，当前求解与整段历史回测均使用这份快照。目标不再按历史生效日期切换。
...
组合优化对所选范围内全部叶证券一次联合求解，用同一个收益样本和协方差同时满足各层 Weight／Risk 目标；不会先各自求解子组合再拼接。每层诊断、最终权重和风险贡献来自同一全局解。整组合与单个 sleeve 运行会明确标识风险归因范围；选中 sleeve 的结果不包含范围外持仓。Cash 和 FCN/Option 仍是模型外资本，固定衍生品与冻结持仓不能为满足目标而自动调仓。目标冲突、硬约束或数据不足会在结果中披露，不以数值收敛代替配置可执行性。

若冻结或显式上下限使目标不能完全命中，满足硬约束且收敛的结果可标为“约束下解”并继续研究模拟，同时保留目标偏差；这不表示目标已达成或获得全局最优证明。没有有效风险分母、硬约束不满足或未收敛的目标偏差不能作为可执行解。

“优化参数”位于“运行优化”左侧，弹窗分为范围与数据、组合求解、回测执行。修改只保存在本地草稿中；保存成功后生效，取消、关闭或 Escape 均丢弃未保存修改。优化配置摘要显示所选分类、数据截止日、实际波动率上限或目标及调仓频率。

运行前需要确认分类目标、资金模式、顶层分类权重上下限、不交易分类、调仓频率、比较基准、现金收益、交易成本与执行延迟。风险估计窗口、协方差和缺失收益政策沿用组合设置，不在优化页重复配置；数据截止日不改变当前目标配置。历史资产及需要人工复核的结果以保存记录的状态披露。

求解结果使用一张可逐级展开的分类树表，显示目标风险贡献、求解风险贡献、账面金额、当前权重、目标权重和调仓金额。证券账面金额为当前市值，衍生品沿用带符号账面金额；当前权重为账面金额占组合净资产的比例，目标权重为求解资金占组合净资产的比例。调仓金额显示目标资金与当前账面的带符号差额，分类行汇总直接成员，不重复计算下级。选择“导出 Excel”可下载完整层级和原始数值，不受折叠状态影响。旧结果按保存时的分类和口径显示；缺失且不能从存档可靠推导的字段保持空白，不以当前数据补齐。

下方集中显示“真实组合与回测”：真实收益使用 Performance 的时间加权收益，剔除申购赎回影响；回测从组合成立日的真实收盘持仓、现金和衍生品资本开始，不重复收取初始买入费用；成立日前行情只用于风险预热。风险数据不足时保留初始持仓，等数据足够后才按目标调仓，即使零延迟，首次交易也晚于成立日收盘。初始持仓估值或行情起点缺失时明确不可用，不换成现金或后来持仓。随后不重放真实普通交易和申赎；衍生品仍按已披露的固定资本生命周期假设处理。图表可切换收益指数和回撤；指标使用共同有效收盘区间，实际、回测及可选基准的比较分母一致。月度矩阵按年份分组，分别列出真实、回测、差值及可选基准，横向包含1–12月和年度累计收益；各自保留全部可靠历史，只有起止日期一致时才计算差值。年度收益复合计算，不是月收益相加，也不是年化收益。指标包括区间收益、满周年年化收益、波动率、Sharpe、Sortino、Calmar、最大及当前回撤、回撤起止和修复天数；非完整月份另有提示。子分类模拟与全组合实际并非同一覆盖范围，页面会明确提示。

月度收益矩阵下方默认展示回测一级分类权重及累计收益贡献，两图使用同一分类配色，保留负现金与负权重。求解诊断、运行日期、结果分类、历史数据覆盖和成本保留在默认折叠的“运行审计”内。历史模拟按所设现金收益、佣金、税费、滑点与执行延迟计算。保存的旧运行继续可读，但不再以第二组重复图表展示。

组合优化失败不会覆盖上一轮成功结果。失败后先看页面上的失败原因，再检查 Taxonomies、TargetSet、缺失收益政策、行情覆盖、benchmark 和 top sleeve bounds。不要为通过求解而随意放宽约束；约束应反映真实投资纪律。

页面先读取紧凑运行摘要，再加载最近保存的完整结果；切换组合时，旧请求不会覆盖当前组合。
...
4. Portfolio Overview、Performance、Risk 或组合优化中搜索并选择该指数作为 benchmark。
...
1. 在分类页核对标的归属、各层配置依据和战略／战术目标。
2. 打开组合优化的“优化参数”，设置分类、资金模式、基准、调仓频率、成本和执行约束并保存。
3. 点击“运行优化”，检查树表中的目标与求解风险贡献、目标权重及调仓金额。
4. 对照真实组合与回测、月度矩阵和分类图表；必要时展开运行审计。
5. 把结果作为调仓讨论依据，不直接当作已执行交易。
...
- 组合优化失败：检查 TargetSet、scope assignment、缺失收益政策、benchmark 行情和 top sleeve bounds。
- 保存目标后组合优化仍显示旧结果：检查过期标识并重新运行；研究与回测始终使用最新保存的目标，已保存的旧结果不会被自动改写。
