...
目标页面的一次保存会整体提交本次修改，并自动安排受影响的组合重算。重新计算期间页面可能短暂显示等待；保存失败则本次目标不会部分生效。既有 Research 结果保留当时的输入和结论，并标记过期；需要在 Research 点击运行，才能取得使用新目标的求解结果。
...
动态研究分别显示估值日和规划日期：持仓及行情截止真实可用估值日，目标使用团队当日已经生效的配置，休市或行情较旧不会让刚保存且已生效的目标消失。固定分析日的研究仍使用对应日期的配置，未来生效目标不会提前采用。交易、目标、分析设置或所需行情更新后，旧结果会提示过期；计算过程中输入改变会终止本次发布，保留上一份有效结果。
...
- 保存目标后 Research 仍显示旧结果：检查该结果的过期标识、规划日期及目标生效日，再重新运行；已保存的旧研究不会被自动改写。
